...
router.get('/', async (req, res) => {
  try {
    ...
    const properties = await db.prepare('SELECT * FROM properties ORDER BY created_at DESC').all();
    const result = [];
    for (const p of properties) {
      const links = await db.prepare('SELECT * FROM platform_links WHERE property_id = ? ORDER BY linked_at DESC').all(p.id);
      result.push({ ...p, links });
    }
...
router.delete('/:id', async (req, res) => {
  try {
    const db = await getDb();
    const property = await db.prepare('SELECT id FROM properties WHERE id = ?').get(req.params.id);
    if (!property) return res.status(404).json({ success: false, error: 'Property not found' });
...
router.post('/:id/links', async (req, res) => {
  ...
  try {
    ...
    const link = await db.prepare('SELECT * FROM platform_links WHERE id = ?').get(info.lastInsertRowid);
...
router.delete('/:id/links/:linkId', async (req, res) => {
  try {
    ...
    await db.prepare('DELETE FROM platform_links WHERE id = ?').run(req.params.linkId);
...
router.post('/:id/manual-stats', async (req, res) => {
  ...
  try {
    ...
    const cl = clicks || 0;
    const rc = reach || 0;
    const existing = await db.prepare('SELECT id FROM analytics_cache WHERE property_id = ? AND platform = ?').get(propertyId, platform);
    if (existing) {
      await db.prepare(`
